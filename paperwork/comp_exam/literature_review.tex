...
%    How do we know when we are approaching equality? We can use a distance metric.
    ...
    \noindent So far, we have seen various definitions of exact and approximate equivalence on deterministic functions. Let us now turn our attention to the probabilistic setting. It is seldom the case two distributions are exactly equal, but rather agree in some places and disagree in others. Rather than verify equality, we need a way to measure similarity. We need a \textit{probability metric}.\\
    ...
    \begin{tikzpicture}[scale=0.6]
        ...
        mark=none,domain=-2.0:1.8,samples=50,smooth},
            ...
            \addplot[name path=F] {gauss(0,0.38)};
    ...
    \noindent Not only do we care about the ``vertical'' distance or relative frequency, but also the ``horizontal'' distance, or \textit{displacement}. First, let us consider the Kantorovich--Rubinstein (KR) metric. In the one-dimensional discrete case, it can be seen as an optimal assignment problem, where $Π$ is the set of all mass-conserving reassignments between two probability vectors $u$ and $v$:

    $$
    δ_{\textsc{KR}}(u, v) = \min_{π\in Π}\Big\{\sum_{i, j} π[i, j]d(x, y) \Bigm| \sum_i π[i, j]=v[j], \sum_j π[i, j] = u[i] \Big\}
    $$

    \noindent We seek $π^*\in Π$ minimizing the total displacement. Generally, $Π$ is a set of feasible assignments or \textit{transport plans} between two distributions $μ$ and $ν$.

    $$
    δ_{\textsc{KR}}(u, v) = \inf_{π\in Π(u, v)}\int_{E\times E} δ(x, y)dπ(i, j)
    $$

    \noindent The Kullback–Leibler (KL) divergence is computationally more efficient, but distinguishes poorly between distributions with non-overlapping support. While asymmetric, it can be symmetrized into the Jensen-Shannon (JS).
   ...
   \end{align*}
    ...
    \noindent Computing these integrals directly grows quickly intractable in higher dimensions. Various approximations such as Gibbs sampling or Markov methods have been developed to alleviate this problem, and are often employed unless the integral in question can be decomposed into lower-dimensional sums and products of conditionally independent random variables (cf. \S~\ref{sec:distributions-graphs}).
