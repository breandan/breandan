...
%    http://users.ox.ac.uk/~scro3229/documents/birmingham-talk.pdf
    ...
    A similar concept also arises in natural languages: ``You shall know a word by the company it keeps''. If two different words can be exchanged in all contexts, they are called \textit{synonyms}. Based on this definition, we can define a context as a probabilistic description of the surrounding environment:

    $$
    C[\![\cdot]\!] := P(w_i = \cdot \mid w_{i-n}, \ldots, w_{i-1}, w_{i+1}, \ldots, w_{i+n})
    $$

    Thus, to determine whether two words $a$ and $b$ have the same meaning in some language $\mathcal L$, we must show that for all contexts $C[\![\cdot]\!] $, $a$ and $b$ have the same probability, i.e. $\forall C[\![\cdot]\!] \in \mathcal L, C[\![a]\!] \approx C[\![b]\!]$. Alternatively, to show $a$ and $b$ have a different meaning, we need only demonstrate a single context where their probability significantly differs, i.e. $\exists C[\![\cdot]\!] \in \mathcal L$ such that $C[\![a]\!] \not\approx C[\![b]\!]$.

    We can define semantic equality in our setting using word embeddings. Interchangable words are semantically equivalent.~\citep{clark2020electra}
