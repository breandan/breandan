...
\begin{document}
    ...
    \section{From exact to approximate knowledge}\label{sec:definitions}

    Reason is the source of all human knowledge. To understand and reason about knowledge, we must first understand how concepts are formed and related. To do so, we must start from first principles: how do humans distill concepts from unstructured data and match new data to prior concepts?

    Data is a sea of smoothly varying quantities and continuous relations, but knowledge is a land of discrete structures and finite relations. To navigate these shores, we must distinguish what is \textit{known} from what is \textit{unknown}. How do we know what is known if the same knowledge can take many forms? We must be able to recognize when two forms are the same or alike.

    Next to identity, one of the simplest relations is a binary relation known as equality. Equality is like identity for forms that may differ in superficial ways, but are the same in all but name and appearance. More precisely, we may care about equality modulo certain properties, like size, orientation or color. We can generalize this to form an \textit{equivalence relation} $\equiv$, i.e. any binary relation which shares the following logical properties:
    ...
    Although it might seem trivial, deciding equivalence is often far from it. Depending on the objects and properties under comparison, equivalence can be undecidable (\S~\ref{sec:algorithms}). In order to carve out decidable fragments, various definitions and decision procedures have been proposed, e.g. intensional (\S~\ref{sec:in-eq}), computational (\S~\ref{sec:comp-eq}), or observational equivalence (\S~\ref{sec:obs-eq}).

    More generally, we have something called a \textit{metric}, which like equality, takes two objects, but instead returns a numerical score. There are a huge number of metrics with various desirable properties (\S~\ref{sec:ap-eq}). Then we have \textit{kernels}, which are metrics with some additional useful properties, e.g. closure under linear combination. Finally, we have probability kernels and metrics, which are distance metrics on probability measures (\S~\ref{sec:pr-eq}).

    We can also define equivalence on other kinds of structures, such as strings or graphs, called a morphism, e.g. graph isomorphism (\S~\ref{sec:morph}).
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{c|l:l}
            ...
            \end{tabular}
        \end{tabular}\label{tab:decision_procedures}
    \end{table}
    \egroup
    ...
    \begin{enumerate}
        ...
        \item Sample: Build a probabilistic model and sample from its posterior
    \end{enumerate}
    ...
    \subsection{Intensional equivalence}\label{sec:in-eq}
...
%        \UnaryInfC{\textit{Commutivity}}
        ...
        \UnaryInfC{$Γ^{n+1}_{g} \vdash \bigcup\limits_{\substack{e \in E\\ω \in Ω}}e'\leftarrow e[ω_1\rightarrow ω_2], (e, e')$}
        \DisplayProof
        \vskip 1em
    ...
    \noindent For example, suppose we are given $f: \{a, b, c\} \mapsto a b c, g: \{a, b, c\} \mapsto c b a$ and $Ω := \{(σ, σ), (στ, τσ)\}$. Indeed, $\textsc{Eq}[f, g]$ can be established as follows:
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Assuming termination, $\mathcal{O}_{\textsc{Eq}} = \Theta_{\textsc{Neq}}$ although $\mathbb{E}[\Theta_\textsc{Eq}|f \equiv g]$ is more tractable. However termination is not necessarily guaranteed, e.g. $Ω' := \{(σ, 1σ)\}$. Equality and termination under arbitrary $Ω$ are known to be undecidable~\citep{baader1999term}.

    \subsection{Computational equivalence}\label{sec:comp-eq}

    Clearly, the procedure defined in \S~\ref{sec:in-eq} is highly sensitive to $|\mathcal{X}|$ and $Ω$. While equality may be tractable, disequality is definitely an obstacle. In the computational setting, we will see the opposite holds, ceteris paribus. %Two values $r, r': \mathcal{Y}$ share a normal form in which equality is trivial.
...
%    Metamorphic testing
    ...
    \pagebreak\subsection{Observational equivalence}\label{sec:obs-eq}

    As presented, both intensional (\S~\ref{sec:in-eq}) and computational (\S~\ref{sec:comp-eq}) equivalence require an external definition of equality to satisfy. One solution to this problem, known as \textit{observational} or \textit{contextual equivalence}~\cite{morris1969lambda}, allows a language $\mathcal{L}$ to implement an internal mechanism to verify equality. Given $\mathcal{L}$, a term $t$, and one-hole context $C[\![\cdot]\!]$, our job is to check for termination: if $C[\![t]\!]$ is both well-defined and halts, we write $C[\![t]\!]\Downarrow$, otherwise $C[\![t]\!]\Uparrow$.
...
%    http://users.ox.ac.uk/~scro3229/documents/birmingham-talk.pdf
    ...
    A similar concept also arises in natural languages under the familiar maxim, ``You shall know a word by the company it keeps.'' Two words which always share the same contexts are semantically equivalent. Two terms which are equiprobable in all contexts, are called \textit{interchangeable}. We can view a context as a probabilistic description of the surrounding environment, e.g., a one-holed linear context can be defined as a stochastic function which accepts a term and returns the probability of its occurrence\footnote{A similar definition may be found in~\cite{clark2020electra}.}:

    $$
    C[\![\cdot]\!] := P_{\mathcal L}(s = \cdot \mid \overset{←}{s}, \overset{→}{s} = [\![, ]\!])
    $$

    Thus, to determine whether two terms $a$ and $b$ share a common meaning in some language $\mathcal L$, we must show that for all contexts $C[\![\cdot]\!] $, $a$ and $b$ are approximately equiprobable, i.e. $\forall C[\![\cdot]\!] \in \mathcal L, C[\![a]\!] \approx C[\![b]\!]$. Alternatively, to show $a$ and $b$ have different meanings, we need only demonstrate some context in which their probability significantly varies, i.e. $\exists C[\![\cdot]\!] \in \mathcal L$ such that $C[\![a]\!] \not\approx C[\![b]\!]$. In the following section, we will try to provide a more precise semantics to this logical operator, $\approx$.

    %However this proposal only works if we have access to $P_{\mathcal L}$, an intractable assumption. Instead we are often in the low-data regime, where we only have an approximation $\hat P_{\mathcal L}$, from which we must reconstruct $P_{\mathcal L}$.

%    We can define semantic equality in our setting using word embeddings.

    \pagebreak\subsection{Approximate equivalence}\label{sec:ap-eq}

%    How do we know when we are approaching equality? We can use a distance metric.

    Though precise, boolean equality is far too rigid for many applications. The spaces involved either lack formal semantics or are intractable to verify, leaving decidability out of reach. Furthermore, we would like to provide a more granular way to distinguish objects. This can be achieved by introducing a new relation for reasoning in discrete and continuous spaces, called a \textit{distance metric}, $δ: \mathcal{Z}\times\mathcal{Z}\rightarrow\mathbb{R}_{\geq 0}$. It has the following logical properties:
    ...
    An \textit{approximate equivalence relation} is a relation $\mathcal{A} \subset \mathcal (\mathcal X → \mathcal Y)^2$,~ e.g. $f \approx_{d} g \Leftrightarrow \mathbb δ_X(f, g) \leq d$, where $X\subseteq \mathcal X$. For example, consider a dataset $X := [x^{(i)}]_{i=1}^n$, $Y := [f(x^{(i)})]_{i=1}^n$ and a program $g: \mathcal{X} → \mathcal{Y}$. We could use the extensional approach to evaluate $δ_X: (f, g) \mapsto δ_Y(Y, [g(x^{(i)})]_{i=1}^n)$ directly.

%    Alternately, given an intensional descriptor $\varphi_θ(\cdot): (\mathcal X → \mathcal Y) → \mathcal Z$ we could construct a synthetic kernel $\Delta_θ: (f, g) \mapsto \left<\varphi_θ(f), \varphi_θ(g)\right>$, seeking $θ^* = \text{argmin}_θ\big(\Delta_θ(f, g) - \Delta_X(f, g)\big)^2$ over $f, g \overset{iid}{\sim} P(\mathcal X → \mathcal Y), X \overset{iid}{\sim} P(\mathcal X^n)$ via gradient descent. The problem then becomes: how do we match the distribution of naturally-arising programs?

    It is often the case we are given a dataset $P_{\text{Train}}, P_{\text{Test}}: (X \times Y)^n$ sampled from an inaccessible distribution $P^{(1..n)} \overset{iid}{\sim} P_\text{Gen}(X, Y)$, and want to compare a deterministic computer program $\hat p: X \times \Theta → Y$ approximating $P_\text{Gen}$ to detect errors with respect to that dataset. One mechanism for doing so can be described as follows:\ldots
...
%            https://www.cs.mcgill.ca/~wlh/grl_book/files/GRL_Book.pdf#page=22
    ...
    % However, what is needed is a way to quantify the probability a certain output is produced, or the likelihood of observing a representable function in the wild, given some prior knowledge. To do this properly, we need a language for propagating uncertainty through a computation.

    % https://towardsdatascience.com/beyond-weisfeiler-lehman-approximate-isomorphisms-and-metric-embeddings-f7b816b75751

    \noindent So far, we have seen various definitions of exact and approximate equivalence on deterministic functions. We now turn our attention to the probabilistic setting, where two distributions are seldom exactly equal, but rather agree in some places and disagree in others. How could we measure their similarity? %We need a \textit{probability metric}.\\

    \hspace*{-0.6cm}\begin{tabular}{cccc}
    \begin{tikzpicture}[scale=0.6]
        \begin{axis}[x=2cm, y=2cm, every axis plot post/.append style={
        mark=none,domain=-2.0:1.8,samples=50,smooth},
            axis x line*=bottom, % no box around the plot, only x and y axis
            ticks=none,
            y axis line style={draw=none},
            xticklabels={,,},
            enlargelimits=upper] % extend the axes a bit to the right and top
            \addplot[name path=F] {gauss(0.0,0.38)};
            \addplot[name path=G] {gauss(0.0,0.65)};
            \addplot[pattern=vertical lines, pattern color=gray!50]fill between[of=F and G, soft clip={domain=-2.0:1.8}];
        \end{axis}
        \node [xshift=120pt, yshift=10pt] {\footnotesize $P_1(X)$};
        \node [xshift=40pt, yshift=30pt] {\footnotesize $P_2(X)$};
    \end{tikzpicture}
    & & &
    \begin{tikzpicture}[scale=0.6]
        \begin{axis}[x=2cm, y=2cm, every axis plot post/.append style={
        mark=none,domain=-2:2.5,samples=50,smooth},
            ...
            xticklabels={,,},
            enlargelimits=upper] % extend the axes a bit to the right and top
            \addplot[name path=F] {gauss(0.0,0.5)};
            \addplot[name path=G] {gauss(0.5,0.6)};
            \addplot[pattern=horizontal lines, pattern color=gray!50]fill between[of=F and G, soft clip={domain=-2:0.495}];
            \addplot[pattern=horizontal lines, pattern color=gray!50]fill between[of=F and G, soft clip={domain=0.495:2.5}];
        \end{axis}
        \node [xshift=20pt, yshift=20pt] {\footnotesize $P_1(X)$};
        \node [xshift=130pt, yshift=20pt] {\footnotesize $P_2(X)$};
    \end{tikzpicture} \\
    Frequency & & & Displacement
    \end{tabular} \\

    \noindent We need a way to quantify the ``vertical'' distance or \textit{relative frequency}, as well as the ``horizontal'' distance, or \textit{displacement} between distributions. One family of functions, known as the R\'enyi divergences, estimate the relative frequency of two distributions, $P$ and $Q$ by comparing their relative entropy. For example, consider the Kullback-Leibler (KL) divergence:

    \begin{align*}
%        δ_{\textsc{TV}}(P, Q) &= \int |P(X) - Q(X)|dx
        δ_{\textsc{KL}}(P → Q) &:= \int P(X) \log\frac{P(X)}{Q(X)}dx
%        δ_{\textsc{JS}}(P, Q) &= \frac{1}{2}δ_{\textsc{KL}}(P → M) + \frac{1}{2}δ_{\textsc{KL}}(Q → M)
    \end{align*}

    \noindent Although not strictly metrics, these functions are computationally tractable to estimate, although distinguish poorly between distributions which do not share overlapping support, such as in the following scenario:\\\\

    \begin{tikzpicture}[scale=0.5]
        \begin{axis}[x=2cm, y=2cm, every axis plot post/.append style={
        mark=none,domain=-2:7.5,samples=50,smooth},
            axis x line*=bottom, % no box around the plot, only x and y axis
            ticks=none,
            y axis line style={draw=none},
            xticklabels={,,},
            ...
            \addplot[name path=H] {gauss(6.0,0.5)};
        ...
        \end{axis}
        \node [xshift=57pt, yshift=7pt] {\footnotesize $P_1(X)$};
        \node [xshift=142pt, yshift=7pt] {\footnotesize $P_2(X)$};
        \node [xshift=227pt, yshift=7pt] {\footnotesize $P_3(X)$};
    \end{tikzpicture} \\

    \noindent Notice how $δ_{\textsc{KL}}(P_1 → P_2) \approx δ_{\textsc{KL}}(P_1 → P_3)$, despite $P_3$ being nearly twice as ``far'' as $P_2$. Can we somehow quantify this? Let us now consider the Kantorovich--Rubinstein (KR) metric. Viewed as an optimal transport problem, $δ_{KR}$ minimizes over $Π$, the set of all mass-conserving transportation plans between two probability distributions $μ$ and $ν$, on a sample space $Ω$:

    $$
    δ_{\textsc{KR}}(μ, ν) := \inf_{π\in Π(μ, ν)}\int_{Ω\times Ω} δ(x, y)dπ(x, y)
    $$

%    In the one-dimensional discrete case, it can be seen as an optimal assignment problem,
%
%    $$
%    δ_{\textsc{KR}}(u, v) = \min_{π\in Π}\Big\{\sum_{i, j} π[i, j]d(x, y) \Bigm| \sum_i π[i, j]=v[j], \sum_j π[i, j] = u[i] \Big\}
%    $$

    \noindent The distance between $μ$ and $ν$ is given by $π^*\in Π$ minimizing the total displacement. However integrating these quantities grows computationally intractable in higher dimensions. Various approximations such as Gibbs sampling or Markov methods have been developed to mitigate this problem. % are often employed unless the integral in question can be decomposed into lower-dimensional sums and products of conditionally independent random variables (cf. \S~\ref{sec:distributions-graphs}).
...
%    During test time, we query a dataset for the $k$ most similar functions, and try to unify them computationally and intensionally. We can train a metric $M_\theta$ and discriminator $D_\theta$ on pairs of random functions $f_1$ and $f_2$, to predict their similarity. During inference, we let the discriminator sample random inputs $\hat i_1 \ldots n \sim D_\theta(\hat i \mid f_1, f_2)$ from its latent distribution, conditioning on the structure of $f_1$ and $f_2$. In other words, we want a model that predicts similarity and outputs values which are likely to demonstrate instances of inequality.
    ...
    \subsection{Morphisms}\label{sec:morph}


    Many objects we would like to compare cannot be reduced into a canonical form, making equality challenging. We can define a relation on these objects is called a \textit{morphism}, e.g. graph sequence, morphisms, equivariance. For example, consider two different graphs with the same topology, but different labels. Is there a way to identify equality in this setting?

    Query languages. For strings, we have automata. For relational databases, we have SQL. For code, we have CodeQL / symbolic automata. For graphs we have WL kernels.
    ...
    Many optimization problems can be seen as dual to each other: KKT and SVM duality. Duality occurs in automatic differentiation with dual number arithmetic. Duality also arises in computer science. One well-known example is the duality between code and data: in \textit{homoiconic} languages, we can treat code as data and data as code. Is there a common way to represent both?

    One way to view automatic differentiation is that it allows us to compute the sensitivity of numerical values in a fixed computation graph. What we wanted to compute sensitivities with respect to changes in the computation graph itself? For that, we need to define the graph as an algebraic object.
    ...
    The connection between algebra and graphs runs deep, unifying many seemingly disparate topics. It is possible to implement many fundamental computer science algorithms in a much simpler way as iterated matrix multiplication on a semiring algebra. A commutative monoid $(S, •, \circled{1})$ is a set $S$ with a binary operator $•: S \times S → S$ which has the following properties:
    ...
    \noindent At is possible solve a wide variety of path problems using iterated matrix multiplication on semirings. Known as \textit{propagation} or \textit{message passing}, this procedure consists of two steps: \textit{aggregate} and \textit{update}. Let $δ_{st}$ denote some distance metric on a path between two vertices $s$ and $t$ in a graph. To obtain $δ_{st}$, one may run the following procedure on a desired path algebra until convergence:
    ...
    \pagebreak\subsection{Programs are graphs}\label{sec:program-graphs}
...
\end{adjustbox}
    ...
    Effectively a program is a kind of typed attribute graph. A type is a unique string identifying a set of things which each node accepts. In category theory this is called a commutative diagram, i.e. the transitive closure of a type graph.
...
%    \end{enumerate}
    ...
    A probability distribution is like a random walk on a graph. We can think of a probability distribution as function $P(X)$ measuring the relative frequency which certain values appear in the output of a black box stochastic function $F$, whose internal properties we can only guess by external observation. The intensional description $\hat F(x)$ can be seen as a graph, whose vertices represent the internal states of some dynamical system, and edges represent the probabilities of transitioning between those states.

%    Although we may never access the true graph which generated a given distribution, we can try to reconstruct it by feeding many inputs into a surrogate function $\hat F(X;\theta)$ and updating either the graph topology or transition probabilities so as to more closely match the true distribution. Regardless of whether the intensional descriptions match, if we can do so in a way that causes the extensional descriptions to align, then we say that $F$ and $\hat F$ are \textit{extensionally indistinguishable}.
...
%    \end{tabular}
    ...
    \pagebreak\subsection{Knowledge is a graph}\label{sec:kgs}
...
%    \noindent To increase the speed of type inference, we can precompute the congruence $C[\![\cdot]\!]$ and preimage closure $C^{-1}[\![\cdot]\!]$ so that $\textsc{Infer}$ and $\textsc{Fill}$ are both $\mathcal{O}(1)$. The type tensor simply enumerates paths between types in our type system, but does not tell us how likely a given path is to occur in an arbitrary program. To compute this probability, we need a stochastic transition matrix between functions, $\mathcal{F}\times\mathcal{F}$ representing the probability that some function is mapped to another. We plan to discuss this aspect in future work.
    ...
    Our initial approach described in~\S\ref{sec:kgs}, simply enumerates paths between types in our type system, but does not tell us how likely a given path is to occur in a codebase. To compute this probability, we need a stochastic transition matrix between functions, $\mathcal{F}\times\mathcal{F}$ representing the probability that some function is to be composed with another. Given a grammar and type system, we would like a way of sampling novel programs according to the distribution of programs which occur in some desired codebase.
  ...
  \section{Appendix}
    ...
    For such a fundamental concept, the notation for equality is recklessly overloaded in mathematics and computer science. Depending on the surrounding context, the expression $x = y$ may be used to denote:

    \begin{enumerate}
    \item Define x to be y, e.g. ``Let $x = y$''.
    \item x and y are the same, e.g. ``\ldots therefor, $x = y$''.
    \item For all $x$, $x$ and $y$ are the same, e.g. $a^2 = a\cdot a$
    \item There exists an $x$ such that $x$ and $y$ are the same, e.g. $x^2 = 4$
    \item Store the value of \texttt{y} in the register \texttt{x}, e.g. \texttt{val x = y}
    \item Are x and y the same? e.g. \texttt{if(x = y)}
    \end{enumerate}

%    https://math.stackexchange.com/a/2517357/533073
%    https://math.stackexchange.com/q/1100140/533073

    Identity vs. equality vs. isomorphism

    Thus, we say that the semantics of the expression $x= y$ are \textit{context sensitive}.

    If either (1-3) apply, it should be possible to treat them in the same manner.

    Suppose we need to compute the derivative of a logical function with respect to its inputs (6). The trouble is, logical equality is not differentiable. Consider the Kronecker $δ$-function:
