...
%    How do we know when we are approaching equality? We can use a distance metric.
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
        \node [xshift=55pt, yshift=30pt] {\footnotesize $P_1(X)$};
        \node [xshift=200pt, yshift=30pt] {\footnotesize $P_2(X)$};
    \end{tikzpicture}
    \end{figure}
  ...
  \noindent Now consider the Kantorovich--Rubinstein, or Earthmover metric.

    $$
    d(u, v) = \text{min}\Big\{\sum_{i, j} π[i, j]d(x, y) \Bigm| π[i, j] > 0, \sum_i π[i, j]=B_j, \sum_j π[i, j] = y_j \Big\}
    $$
