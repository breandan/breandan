...
%    How do we know when we are approaching equality? We can use a distance metric.
    ...
    \noindent So far, we have seen various notions of exact and approximate equivalence on deterministic functions. We now turn our attention to the probabilistic setting. It is seldom the case two distributions are exactly equal, but rather align in some places and diverge in others. Rather than verify equality, what is needed is a way to measure similarity. We need a \textit{probability metric}.\\
  ...
  \noindent The Kullback–Leibler (KL) divergence is one way to quantifying the distance between two distributions. Though not a proper metric due to asymmetry, it is computationally efficient to evaluate, and may be defined as follows.
    ...
    \noindent Unlike the KL, the Kantorovich--Rubinstein (KR) metric is a proper metric. Not only do we care about the ``vertical'' distance or relative frequency, but also the ``horizontal'' distance, or \textit{displacement}. In the 1D discrete case, it can be seen as an optimal assignment problem, where $Π$ is the set of all mass-conserving reassignments between two probability vectors $u$ and $v$:
   ...
   \noindent Computing $δ_\textsc{KR}$ directly grows quickly intractable in higher dimensions. Various kernels, metrics and pseudometrics on multidimensional distributions have been proposed to alleviate this problem, e.g. Kolmogorov--Smirnov, Jensen--Shannon, Lévy--Prokhorov, et al. Approximations such as Gibbs sampling or Markov methods are often employed unless the integral in question can be decomposed into lower-dimensional sums and products of conditionally independent random variables (cf. \S~\ref{sec:distributions-graphs}).
