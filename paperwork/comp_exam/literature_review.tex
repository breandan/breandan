...
%    \end{enumerate}
    ...
    We can think of a probability distribution as the output of a stochastic function $F$ which can only be accessed through its extensional description $P(X)$, approximating the relative frequency which certain outputs appear in the codomain. The intensional description $F(x)$ can be seen as a graph, whose nodes represent internal states of the system and whose edges represent the probabilities of transitioning between those states.
