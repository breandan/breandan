...
\begin{document}
    ...
    Indispensable to this process is the ability to cross between a sea of smoothly varying quantities and continuous relations, and a land of discrete structures and finite relations. To navigate these shores, we must be able to distinguish that which is \textit{known} from that which is \textit{unknown}. How do we know what is known if the same knowledge can take many different forms? We must be able to recognize when two forms are the same or alike.
    ...
    More generally, we have something called a metric, which like equality, takes two objects, but instead returns a numerical score. There are a huge number of useful metrics with various desirable properties (\S~\ref{sec:ap-eq}). Then we have kernels, which are distance metrics with some additional useful properties. Finally, we have probability metrics and kernels, which are probabilistic distance metrics on probability measures (\S~\ref{sec:pr-eq}).
...
%    http://users.ox.ac.uk/~scro3229/documents/birmingham-talk.pdf
    ...
    A similar concept also appears in natural languages: ``You shall know a word by the company it keeps''. If two different words can be exchanged in all contexts, they are called \textit{synonyms}. Based on this definition, we can define a context as a probabilistic description of the surrounding environment:

    $$
    C[\![\cdot]\!] = P(w_i = \cdot \mid w_{i-n}, \ldots, w_{i-1}, w_{i+1}, \ldots, w_{i+n})
    $$

    Thus, to determine whether two words $a$ and $b$ have the same meaning in some language $\mathcal L$, we must show that for all sentences $C[\![\cdot]\!] $, $a$ and $b$ have the same probability, i.e. $\forall C[\![\cdot]\!] \in \mathcal L, C[\![a]\!] \approx C[\![b]\!]$. Alternatively, to show $a$ and $b$ have a different meaning, we need only provide a single sentence where their probability significantly differs, i.e. $\exists C[\![\cdot]\!] \in \mathcal L$ such that $C[\![a]\!] \not\approx C[\![b]\!]$.
...
%        δ_{\textsc{JS}}(P, Q) &= \frac{1}{2}δ_{\textsc{KL}}(P → M) + \frac{1}{2}δ_{\textsc{KL}}(Q → M)
    ...
    \noindent Notice how $δ_{\textsc{KL}}(P_1 → P_2) \approx δ_{\textsc{KL}}(P_1 → P_3)$, despite $P_3$ being nearly twice as ``far'' as $P_2$. Can we somehow quantify this? Let us now consider the Kantorovich--Rubinstein (KR) metric, a proper metric. Viewed as an optimal transport problem, $Π$ is the set of all mass-conserving transportation plans between two probability distributions $μ$ and $ν$, on a sample space $Ω$:
