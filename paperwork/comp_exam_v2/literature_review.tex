...
\pagebreak\section{Code Completion and Neural Code Search}\label{sec:autocompletion}
...
\pagebreak\section{Future Directions of Computer Programming}
...
Prior work explored differentiable programming~\cite{considine2019programming}. Differentiability plays a key role in learning, but does not provide the necessary vocabulary to describe human knowledge. In order to capture human knowledge and begin to reason as humans do, programs must be able to express the concept of \textit{uncertainty}. We propose a set of tools and techniques for reasoning about uncertainty in the form of \textit{procedural knowledge}. \textit{Procedural knowledge} is knowledge designed or discovered to facilitate a process. Unlike \textit{relational knowledge} which may be bidirectional, procedural knowledge has a specific direction or goal.

Graphs are a natural representation for both procedural knowledge~\cite{allamanis2017learning} and probabilistic reasoning~\cite{pearl2014probabilistic}, both enabling a large class of useful data transformations and vastly simplifying their implementation on physical machinery. Depending on the author's perspective, it is possible to view the evaluation of these routines as either operating a stateful machine, reducing a symbolic expression or running a dynamical process on a dataflow graph.

To reason about procedural knowledge, we need a way to compare two procedures for equality. Although known to be undecidable in most languages, various equivalence tests and semi-decision procedures have been developed. For example, we could rewrite said procedures~\cite{baader1999term}, compare them in various contexts~\cite{felleisen1990expressive}, and simulate or execute them on various input data~\cite{chen2020metamorphic} so as to ascertain their exact relationship.
...
One way of ensuring that our values are faithfully encoded by machines is through \textit{programming}. In this literature review, we give examples of various programming models that have been envisioned, e.g. from autocompletion and code search (\S\ref{sec:autocompletion}), to autoprogramming and type inference (\S\ref{sec:automatic-and-declarative-programming}). Another way to imbue human values is by teaching the humanities: arts, language, culture, history and philosophy. By keeping these traditions alive, we transmit our cultural heritage to future generations so that they, by studying the lessons of our past, may avoid repeating our past mistakes. These individuals will soon be able to share our knowledge with computers using their mother tongue.
