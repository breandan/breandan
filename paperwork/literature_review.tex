...
\noindent If we observe an event $Y$ in a joint distribution where $0 < P(Y)$, we can divide the joint by the prior to obtain the conditional distribution:
...
%
    ...
    \noindent Semirings are known to have many useful applications in graph theory and formal languages. Any algebraic structure forming a semiring can be propagated through the graph topology using ordinary matrix multiplication. By selecting the right semiring algebra, we can solve a wide variety of graph problems and program analysis tasks.
