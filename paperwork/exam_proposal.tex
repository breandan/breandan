...
%    Still, we can answer many questions exactly. If we restrict the reasoning system to incomplete queries, we can be consistent.
    ...
    Let $\Omega \subseteq \mathcal{F} \times \mathcal{F}$ be a relation on representable functions which are closed under composition. We say two representations $f, g \in \mathcal{X} \rightarrow \mathcal{Y}$ are intensionally equal under $\Omega$ if we can establish that $g \in \Omega^n(f)$ for some $n \in \mathbb{N}$.

    \begin{prooftree}
        \AxiomC{$f, g: \mathcal{X \rightarrow Y} \in \Gamma^0_{g}$}
        \RightLabel{\textsc{Init}}
        \UnaryInfC{$\Gamma^0_{g} \vdash \{f\}, \{(f, f)\}$}
%        \UnaryInfC{\textit{Commutivity}}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma^{n}_{g} \vdash E \subseteq \mathcal{F}, G \subseteq E \times E$}
        \RightLabel{\textsc{Sub}}
        \UnaryInfC{$\Gamma^{n+1}_{g} \vdash \bigcup\limits_{\substack{e \in E\\\sigma \in \Omega}}e'\leftarrow e[\sigma_1\rightarrow\sigma_2], (e, e')$}
        \DisplayProof
        \vskip 1em
        \AxiomC{$\Gamma^n_{g} \vdash E, G$}
        \AxiomC{$g \in E$}
        \RightLabel{\textsc{Eq}}
        \BinaryInfC{$\Gamma^n_{g} \vdash f \equiv_\Omega g$ by $G^{-n}(g)$}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma^n_{g} \vdash E$}
        \AxiomC{$\Gamma^{n+1}_{g}\vdash E$}
        \AxiomC{$g\notin E$}
        \RightLabel{\textsc{Neq}}
        \TrinaryInfC{$\Gamma^{n+1}_{g} \vdash f \not\equiv_\Omega g$}
    \end{prooftree}

    \noindent For example, suppose we are given $f: \{a, b, c\} \mapsto a b c, g: \{a, b, c\} \mapsto c b a$ and $\Omega := \{(a, a), (ab, ba)\}$. Indeed, $\textsc{Eq}[f, g]$ can be established as follows:

    \vspace{-10pt}\begin{prooftree}
        \def\fCenter{\ := \ }
        \Axiom$f \fCenter abc, \text{ } g := cba \in \Gamma_g^0$
        \def\fCenter{\ \vdash\ }
        \RightLabel{\textsc{Init}}
        \UnaryInf$\Gamma^0_{g} \fCenter \{abc\}, \{(abc, abc)\}$
        \RightLabel{\textsc{Sub}}
        \UnaryInf$\Gamma^1_{g} \fCenter \{\ldots, bac, acb\}, \{\ldots, (abc, bac), (abc, acb)\}$
        \RightLabel{\textsc{Sub}}
        \UnaryInf$\Gamma^2_{g} \fCenter \{\ldots, bca, cab\}, \{\ldots, (bac, bac), (acb, acb), (bac, bca), (acb, cab)\}$
        \RightLabel{\textsc{Sub}}
        \UnaryInf$\Gamma^3_{g} \fCenter \{\ldots, \mathbf{cba}\}, \{\ldots, (bca, bca), (cab, cab), (cab,\textbf{cba})\}$
        \RightLabel{\textsc{Eq}}
        \UnaryInf$\Gamma^3_{g} \fCenter f \equiv_\Omega g\text{ by } G^{-3}(g:=cba) = \{f := abc\}$
    \end{prooftree}
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Let us suppose $|\mathcal{X}|, |\Omega^*| \in \mathbb{N}$ and consider the complexity of establishing $\textsc{Eq}[f, g], \forall f \equiv_\Omega g \in \mathcal{X} \rightarrow \mathcal{Y}$. It can be shown the above procedure requires:

    $$\mathcal{O}_{\textsc{Eq}} = \underset{i \leq n}{\text{max }}\underset{n}{\text{argmin}}\{|G| \mid \Gamma^i_{g} \vdash G, \Gamma_{g}^n = \Gamma_{g}^{n+1}\}$$ % \text{, or } \Theta_{\textsc{Eq}}(|\mathcal{X}|!) \text{ for } \Omega.$$

    \noindent Assuming termination, $\mathcal{O}_{\textsc{Eq}} = \Theta_{\textsc{Neq}}$ although $\mathbb{E}[\Theta_\textsc{Eq}|f \equiv g]$ is more tractable. However termination is not necessarily guaranteed, e.g. $\Omega' := \{(a, 1a)\}$. Equality and termination under arbitrary $\Omega$ are known to be undecidable~\citep{baader1999term}.

    \subsection{Computational equivalence}\label{subsec:comp-eq}

    Clearly, the procedure defined in \S~\ref{subsec:in-eq} is highly sensitive to $|\mathcal{X}|$ and $\Omega$. While equality may be tractable, disequality is definitely an obstacle. In the computational setting, we will see the opposite holds, ceteris paribus. %Two values $r, r': \mathcal{Y}$ share a normal form in which equality is trivial.

    \begin{prooftree}
        \AxiomC{$fg: \mathcal{X} \rightarrow \mathcal{Y}, \Omega: \{\mathcal{X}\rightarrow \mathcal{Y}\}\rightarrow \mathcal{Y} \in \Gamma$}
...
%        \AxiomC{$i: \mathcal{X}$}
        ...
        \DisplayProof
        \vskip 1em
        \AxiomC{$\Gamma, \Gamma' \vdash f(\Omega) \Downarrow g(\Omega) \text{ } \forall \Omega$}
        \RightLabel{\textsc{Eq}}
        \UnaryInfC{$\Gamma, \Gamma' \vdash f \equiv g$}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma, \Gamma' \vdash \exists \Omega \mid f(\Omega) \not\Downarrow g(\Omega) $}
        \RightLabel{\textsc{Neq}}
        \UnaryInfC{$\Gamma, \Gamma' \vdash f \not\equiv g$ by $\Omega$}
    \end{prooftree}

    % big step semantics
%    Extensional equality asks``Do $f_1$ and $f_2$ behave in the same way over all inputs?''
    \noindent \textsc{Sub} loosely corresponds to $\eta$-reduction in the untyped $\lambda$-calculus, however $f \notin \Omega$ is disallowed and we assume all variables are bound by \textsc{Inv}. Let us consider $f: \{a, b, c\}\mapsto abc, g: \{a, b, c\} \mapsto ac$ under $\Omega:=\{(a, 1), (b, 2), (c, 2)\}$:
    ...
    \vspace{-10pt}\begin{prooftree}
        ...
        \def\fCenter{\ :=}
        \Axiom$g\fCenter ac, \Omega:=\{(a, 1), (b, 2), (c, 2)\} \in \Gamma'$
        \def\fCenter{\ \vdash\ }
        \RightLabel{\textsc{Inv}}
        ...
        \RightLabel{\textsc{Sub}}
        \UnaryInf$\Gamma' \fCenter g(\Omega) \Downarrow 2$
        \RightLabel{\textsc{Neq}}
        \BinaryInfC{$\Gamma, \Gamma' \vdash f \not\equiv g$ by $\Omega :=\{(a, 1), (b, 2), (c, 2)\}$}
    \end{prooftree}

    \noindent We can view the above process as acting on a dataflow graph, where \textsc{Inv} backpropagates $\Omega$, and \textsc{Sub} returns concrete values $\mathcal{Y}$, here depicted on $g$:
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Assuming $f, g \sim P(\mathcal{F}), \Omega \overset{iid}{\sim} P_\textsc{Test}(\Omega \mid f \not\equiv g)$ yields a fixed but unknown distribution, $P_\textsc{Neq}(\Omega) = P(f(\Omega)\not\Downarrow g(\Omega) \mid f \not\equiv g)$. Let $\Theta_\textsc{Inv} = 1$ for all $f, \Omega$. The complexity of certifying $\textsc{Neq}$ in $n$ trials follows a geometric distribution:
    ...
    \noindent Although a single witness $\Omega$ s.t. $f(\Omega) \not\Downarrow g(\Omega)$ is sufficient for disequality, this procedure may be intractable depending on $|\mathcal{X}|, P_\textsc{Neq}(\Omega)$ and $\Theta_{\textsc{Inv}}$. Other fuzzing methods for selecting $\Omega$ based on the structure of $f$ are also possible.
...
%    Metamorphic testing
    ...
    \begin{prooftree}
        \AxiomC{$\Gamma \vdash C[\![t]\!]\Downarrow \iff C[\![t']\!]\Downarrow$ $\forall$ $C[\![\cdot]\!] \in \mathcal{L}$}
        \RightLabel{\textsc{Eq}}
        \UnaryInfC{$\Gamma \vdash t \equiv_{\mathcal{L}} t'$}
        \DisplayProof
        \vskip 1em
        \AxiomC{$\Gamma \vdash \exists C[\![\cdot]\!]\in \mathcal{L} \mid C[\![t]\!]\Uparrow $ and $ C[\![t']\!]\Downarrow$, or $C[\![t]\!]\Downarrow $ and $ C[\![t']\!]\Uparrow$}
        \RightLabel{\textsc{Neq}}
...
%    We can define semantic equality in our setting using word embeddings.
    ...
    Approximate equivalence requires a \textit{distance metric}, $\delta: \mathcal{Z}\times\mathcal{Z}\rightarrow\mathbb{R}_{\geq 0}$, a generalized equivalence relation with the following logical properties:

    \begin{prooftree}
        \bottomAlignProof
        \AxiomC{$\delta(a, b) = 0$}
        \UnaryInfC{$a \equiv_\delta b$}
        \noLine
        \UnaryInfC{}
        \noLine
        ...
        \AxiomC{$\delta(a, c)$}
        \AxiomC{$b$}
        \BinaryInfC{$\delta(a, c) \leq \delta(a, b) + \delta(b, c)$}
        \noLine
        \UnaryInfC{}
        \noLine
    ...
    \noindent A \textit{kernel function} $\Delta: (\mathcal{X}\rightarrow\mathcal{Z}) \times (\mathcal{X}\rightarrow\mathcal{Z}) \rightarrow \mathbb{R}$ can be defined as a metric over $\mathcal{Z}$ with some additional structure. In particular for every kernel, there exists a feature map $\varphi: (\mathcal{X} \rightarrow \mathcal{Y}) \rightarrow \mathcal{Z}$ such that $\Delta(f, g) \mapsto \left<\varphi(f), \varphi(g)\right>_\mathcal{Z}$. Given a feature map $\varphi$, we can construct the corresponding kernel as follows:
    ...
    \begin{prooftree}
        ...
        \UnaryInfC{}
        \noLine
        \UnaryInfC{\textit{TODO}}
        \DisplayProof
        \hskip 1.5em
    \end{prooftree}

    For each kernel, there is a primal and dual form ...
    ...
    We want a want a kernel function $M_\theta: (\mathcal{X}\rightarrow \mathcal{Y}) \times (\mathcal{X}\rightarrow \mathcal{Y})\rightarrow \mathbb{R}$ between two computable functions, which predicts their semantic similarity. In other words, the closer two functions are with respect to $M_\theta$, the more likely they are to be equal...

%    It is often the case we are given a dataset $P_{\text{Train}}, P_{\text{Test}}: (X \times Y)^n$ sampled from an inaccessible distribution $P^{(1..n)} \overset{iid}{\sim} P_\text{Gen}(X, Y)$, and want to compare a deterministic computer program $\hat p: X \times \Theta \rightarrow Y$ approximating $P_\text{Gen}$ to detect errors with respect to that dataset. One mechanism for doing so can be described as follows:...

%    \begin{prooftree}
%        \AxiomC{$\Gamma \vdash f \circ g, f: Z \rightarrow Y, g: X \rightarrow Z, B: := P_\text{Train}[i]$}
...
%    \end{prooftree}
    ...
    %Given an oracle $M: \mathcal I \rightarrow \mathcal R$, and a set of inputs $X := \{x^{(i)}\}_{i=1}^n$ and outputs $Y := \{y^{(i)} := M(x^{(i)})\}_{i=1}^n$, a metamorphic relation (MR) is a relation $M \subset \mathcal I^z \times \mathcal R^z$ where $z \geq 2$. In the simplest case, an MR is an equivalence relation $\mathcal R$, i.e.: $\langle \mathbf x, \mathbf y, \mathbf x', \mathbf y' \rangle \in \mathcal R \Leftrightarrow \mathbf x \sim_{\mathcal R} \mathbf x' \Leftrightarrow \mathbf P(\mathbf x) \approx \mathbf P(\mathbf x')$.
