...
%    We can define semantic equality in our setting using word embeddings.
    ...
    \noindent A \textit{kernel function} can be defined as a metric on $\mathcal{Z}$ with some additional structure. In particular for every kernel function, there exists a feature map $\varphi: \mathcal{Y} \rightarrow \mathcal{Z}$ such that $Δ: (a, b) \mapsto \left<\varphi(a), \varphi(b)\right>$. Given a specific feature map $\symbf\varphi: \mathbb{R}^m \rightarrow \mathbb{R}^n$, constructing a kernel corresponds to finding $\Delta \in \mathbb{R}^{m \times m}$ symmetric PSD, i.e. $\Delta^\intercal = \Delta$, and $0 \leq \mathbf{x}^\intercal \Delta \mathbf{x}$ for all $\mathbf{x} \in \mathbb{R}^m$~\cite{mercer1909functions}. Consider the complexity of evaluating the inner product $\left<\symbf\varphi(f), \symbf\varphi(g)\right>$ in feature space. The computational benefit of a kernel becomes apparent when $m \ll n$: rather than applying $\symbf\varphi$ and then computing $\left<f^\intercal\symbf\varphi, g^\intercal\symbf\varphi\right>$, we can simply apply $\Delta(f, g)$ for a $\Theta((m-n)^2)$ speedup. This shortcut may be easier to visualize as a commutative diagram:

    \[\begin{tikzcd}
          \mathcal{Y\times \mathcal{Y}} \arrow{r}{\varphi} \arrow[labels=below left]{dr}{\Delta} & \mathcal{Z}\times\mathcal{Z} \arrow{d}{\left<\cdot, \cdot\right>} \\
          & \mathbb{R}_{\geq 0}
    \end{tikzcd}
    \]

    Consider the space of valid kernel functions, $▲ \subset (\mathcal{Y}\rightarrow\mathcal{Z})^2 \rightarrow \mathbb{R}_{\geq 0}$. We may define it inductively:
