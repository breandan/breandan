...
%    We require a metric~\ref{sec:probabilistic}.
    ...
    Various approaches have been taken to decide exact and approximate equality in the deterministic and probabilistic setting. We list a few below.
    ...
    In order to determine whether two expressions are equal, we require a decision procedure. It is seldom the case that two equal expressions are trivially equal: we must first perform some computation in order to establish their equality. For the case of exact equality, this usually entails a symbol manipulation procedure, which might be summarized as follows:
