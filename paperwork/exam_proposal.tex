...
%    Still, we can answer many questions exactly. If we restrict the reasoning system to incomplete queries, we can be consistent.
    ...
    Let $\Omega \subseteq \mathcal{F} \times \mathcal{F}$ be a relation on representable functions which are closed under composition. We say two representations $f, g \in \mathcal{I} \rightarrow \mathcal{R}$ are intensionally equal under $\Omega$ if we can establish that $g \in \Omega^n(f)$ for some $n \in \mathbb{N}$.

    \begin{prooftree}
        \AxiomC{$f, g: \mathcal{I \rightarrow R} \in \Gamma^0_{g}$}
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Let us suppose $|\mathcal{I}|, |\Omega^*| \in \mathbb{N}$ and consider the complexity of establishing $\textsc{Eq}[f, g], \forall f \equiv_\Omega g \in \mathcal{I} \rightarrow \mathcal{R}$. It can be shown the above procedure requires:

    $$\mathcal{O}_{\textsc{Eq}} = \underset{i \leq n}{\text{max }}\underset{n}{\text{argmin}}\{|G| \mid \Gamma^i_{g} \vdash G, \Gamma_{g}^n = \Gamma_{g}^{n+1}\}$$ % \text{, or } \Theta_{\textsc{Eq}}(|\mathcal{I}|!) \text{ for } \Omega.$$
    ...
    \begin{prooftree}
        \AxiomC{$fg: \mathcal{I} \rightarrow \mathcal{R}, \Omega: \{\mathcal{I}\rightarrow \mathcal{R}\}\rightarrow \mathcal{R} \in \Gamma$}
...
%    Extensional equality asks``Do $f_1$ and $f_2$ behave in the same way over all inputs?''
    ...
    \noindent We can view the above process as acting on a dataflow graph, where \textsc{Inv} backpropagates $\Omega$, and \textsc{Sub} returns concrete values $\mathcal{R}$, here depicted on $g$:
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Although a single witness $\Omega$ s.t. $f(\Omega) \not\Downarrow g(\Omega)$ is sufficient for disequality, this procedure may be intractable depending on $|\mathcal{I}|, P_\textsc{Neq}(\Omega)$ and $\Theta_{\textsc{Inv}}$. Other fuzzing methods for selecting $\Omega$ based on the structure of $f$ are also possible.
...
%    We can define semantic equality in our setting using word embeddings.
    ...
    It is often the case we are given a dataset $p_{\textsc{Train}}, p_{\textsc{Test}}: (X \times Y)^n$ sampled from an inaccessible distribution, where $p^{(1..n)} \overset{iid}{\sim} P(X, Y)$, and want to compare a deterministic computer program $\hat p: X \times \Theta \rightarrow Y$ approximating $P_\textsc{Gen}$ to detect errors with respect to that dataset.
