...
%        \UnaryInfC{\textit{Commutivity}}
    ...
    \begin{prooftree}
        ...
        \RightLabel{Sub}
        \UnaryInf$\Gamma^1_{f} \fCenter \{xyz, yxz, xzy\}, \{(xyz, xyz), (xyz, yxz), (xyz, xzy)\}$
        \RightLabel{Sub}
        \UnaryInf$\Gamma^2_{f} \fCenter \{xyz, \ldots, yzx, zxy\}, \{(xyz, xyz), \ldots, (yxz, yzx), (xzy, zxy)\}$
        \RightLabel{Sub}
        \UnaryInf$\Gamma^3_{f} \fCenter \{xyz, \ldots, zxy, \mathbf{zyx}\}, \{(xyz, xyz), \ldots, (yxz, yzx), (zxy,zyx)\}$
%        \RightLabel{Eq}
%        \UnaryInf$\Gamma^3_{f} \fCenter E \hskip 3em f' \vdash zyx \in E$
        \RightLabel{Eq}
        \UnaryInf$\Gamma^3_{f} \fCenter f=f'\text{ by }f' \subseteq G(zyx) \subseteq G^{2}(zxy) \subseteq G^{3}(xzy) \subseteq xyz = f$
    \end{prooftree}

    Let us consider the complexity of checking $g\overset{?}{=}g'$ for arbitrary $g, g' \in \mathbb{Z}^k$ with respect to $k$. It is possible to show that $\Sigma$ will terminate in

    $$\underset{g' \in \mathbb{Z}^k}{\text{max }}\{|E| \dashv \Gamma_g^n(g, g') = \Gamma_g^{n+1}(g, g')\} = \mathcal{O}(k!)$$

    Now let us consider the modification $\Sigma' := \Sigma \cup \{(a, 1a)\}$. It is possible to show that $\Sigma'$ no longer terminates for any $g, g' \in \mathbb{Z}^k$.
