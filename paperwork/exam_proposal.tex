...
%        \UnaryInfC{\textit{Commutivity}}
        ...
        \AxiomC{$f' \in E$}
        \RightLabel{\textsc{Eq}}
        \BinaryInfC{$\Gamma^n_{f} \vdash f \equiv_\Omega f'$ by $G^{-n}(f')$}
        \DisplayProof
        \hskip 1em
...
%    a -> a [label="Σ₀"]
    ...
    $$\mathcal{O}_{\textsc{Eq}} = \underset{i \leq n}{\text{max }}\underset{n}{\text{argmin}}\{|G| \mid \Gamma^i_g \vdash G, \Gamma_g^n = \Gamma_g^{n+1}\}$$ % \text{, or } \Theta_{\textsc{Eq}}(|\mathcal{I}|!) \text{ for } \Omega.$$
...
%    Extensional equality asks``Do $f_1$ and $f_2$ behave in the same way over all inputs?''
    ...
    \vspace{-10pt}\begin{prooftree}
        ...
        \RightLabel{\textsc{Neq}}
        \BinaryInfC{$\Gamma, \Gamma' \vdash f \not\equiv f'$ by $\Omega :=\{(a, 1), (b, 2), (c, 2)\}$}
    \end{prooftree}
