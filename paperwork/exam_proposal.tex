...
%    We can define semantic equality in our setting using word embeddings.
    ...
    \noindent A \textit{kernel function} can be defined as a metric on $\mathcal{Z}$ with some additional structure. In particular for every kernel function, there exists a feature map $\varphi: (\mathcal{X} \rightarrow \mathcal{Y}) \rightarrow \mathcal{Z}$ such that $Δ: (f, g) \mapsto \left<\varphi(a), \varphi(b)\right>$. Consider a space of valid kernel functions, $▲ \subset (\mathcal{Y}\rightarrow\mathcal{Z})^2 \rightarrow \mathbb{R}_{\geq 0}$. We may define it inductively:
...
%    http://www.cs.cmu.edu/~aarti/Class/10701_Spring14/slides/kernel_methods.pdf#page=38
    ...
    \begin{prooftree}
        ...
        \hskip 0.6em
        \bottomAlignProof
        \AxiomC{$Δ \in ▲, f \in (^*→\mathcal{Y})^2$}
        \UnaryInfC{$Δ\circ f \in ▲$}
    \end{prooftree}

    \noindent Suppose we are given a feature map $\varphi: \mathbb{R}^n \rightarrow \mathbb{R}^m$ and consider the complexity of computing  the inner product $\left<\varphi(f), \varphi(g)\right>$. The computational benefit of a kernel emerges when $n \ll m$: instead of applying the map and computing the inner product, we can instead apply the kernel function directly in input space. This may be easier to visualize as a commutative diagram:

    \[\begin{tikzcd}
          \mathcal{X}_1\arrow{r}{f} & \mathcal{Y\times \mathcal{Y}} \arrow{r}{\varphi} \arrow[labels=below left]{dr}{\Delta} & \mathcal{Z}\times\mathcal{Z} \arrow{d}{\left<\cdot, \cdot\right>} \\
          \mathcal{X}_2\arrow[labels=below right]{ur}{g} & & \mathbb{R}_{\geq 0}
    \end{tikzcd}
    \]

    \noindent Some common kernel functions are listed below:
...
%    \begin{prooftree}
%        \AxiomC{$Γ \vdash f \circ g, f: Z \rightarrow Y, g: X \rightarrow Z, B: := P_\text{Train}[i]$}
