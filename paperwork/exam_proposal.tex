...
%    \end{algorithm}
    ...
    Abduction is the process of knowledge formation: recognizing \textit{patterns} or \textit{motifs} in raw data, forming discrete \textit{categories} or \textit{types} of objects which share those attributes, and adding links which represent abstract relations between them, such as composition, causation, or subsumption. The key idea is moving from the space of unstructured data, to a structured domain containing abstract concepts and their relations. This domain is a graph.

    Knowledge graphs~\citep{hogan2020knowledge} are \textit{multi-relational graphs}, or \textit{hypergraphs} whose edges are typed. Two entities can be related by multiple types, and each type can relate many pairs of entities. This object can be represented by a rank-3 boolean tensor, $\mathbb{B}^3$, which we call a \textit{type tensor}, also known as a \textit{data cube}, although technically a rectangular prism depending on the number of types, $|\mathcal{T}|$. We depict this structure below:
    ...
    \begin{table}[H]
        \centering
        \begin{tabular}{cc}
            \textbf{Typed Graph} & \textbf{Data Cube} \\
...
%    a -> a [label="Σ₀"]
        ...
        \begin{scope}[tdplot_main_coords]
            ...
            (3,0) -- (3,3) node [black,midway,xshift=-8pt, yshift=-14pt] {\footnotesize $\mathcal{O}$};
            \draw [decorate,decoration={calligraphic brace,amplitude=10pt},yshift=0pt, line width=1.25pt]
            (3,0) -- (0,-3) node [black,midway,xshift=-16pt, yshift=0pt] {\footnotesize $\mathcal{O}$};
            \draw [decorate,decoration={calligraphic brace,amplitude=10pt},yshift=0pt, line width=1.25pt]
