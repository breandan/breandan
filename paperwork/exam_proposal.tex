...
%    We require a metric~\ref{sec:probabilistic}.
    ...
    \begin{enumerate}
        \item Rewrite: Either enumerate valid rewrites using enumerative search or reduce the proposition into normal form if possible, then,
...
%        \UnarbInf$\Gamma^3_{f} \fCenter E \hskip 3em f' \vdash cba \in E$
    ...
    \hspace{12pt}\digraph[scale=0.5, center=true]{abcint}{
...
%    Extensional equality asks``Do $f_1$ and $f_2$ behave in the same way over all inputs?''
    ...
    \hspace{-30pt}\digraph[scale=0.5, center=true]{abcext0}{
    node[ fontname="CMU Classical Serif" fontsize=20 ];
    edge[ fontname="CMU Classical Serif" fontsize=18 ];
    rankdir=LR;
    len=3;
...
%    a -> a [label="Σ₀"]
    ...
    d -> a [label="Ω"]
    d -> c [label="Ω"]
    }\hspace{-20pt}\digraph[scale=0.5, center=true]{abcext1}{
    node[ fontname="CMU Classical Serif" fontsize=20 ];
    edge[ fontname="CMU Classical Serif" fontsize=18 ];
    rankdir=LR;
    len=3;
...
%    a -> a [label="Σ₀"]
    ...
    \noindent Suppose $\mathcal{O}_\textsc{Inv}(1)$ for all $f, \Omega$. Assuming $f, f' \overset{iid}{\sim} P(\mathcal{F}), \Omega \overset{iid}{\sim} P_\textsc{Test}(\Omega \mid f, f')$ gives a fixed but unknown distribution $P_\textsc{Neq}(\Omega) = P(f(\Omega)\not\Downarrow f'(\Omega) \mid f\neq f')$. The complexity of $\Theta_\textsc{Neq}$ can be shown to follow a geometric distribution:

    \vspace{-10pt}$$(1 - P_\textsc{Neq}(\Omega))^nP_\textsc{Neq}(\Omega) \text{ with mean } \mathop{\mathbb{E}}[\Theta_\textsc{Neq}] = (1-P_\textsc{Neq}(\Omega))P_\textsc{Neq}(\Omega)^{-1}$$

    \noindent While a single witness $\Omega$ s.t. $f(\Omega) \not\Downarrow f'(\Omega)$ is sufficient for disequality, in general this procedure can be intractable depending on $|\mathcal{I}|, P_\textsc{Neq}(\Omega), \mathcal{O}_{\textsc{Inv}}$.
...
%    Metamorphic testing
    ...
    As constructed, both intensional (\S~\ref{subsec:in-eq}) and computational (\S~\ref{subsec:comp-eq}) definitions are implicitly circular, requiring an external notion of equality. Is there any way to fix this problem, avoiding all reference to inclusion? What if we had a way to observe certain intermediate effects?
...
%    http://users.ox.ac.uk/~scro3229/documents/birmingham-talk.pdf
    ...
    The definition of computational equivalence requires too much work to demonstrate equality, and is too prone to false alarms. It is often the case we are given a partial specification for a program, and may want to check for conformance to a specification.
    ...
    \subsection{Probabilistic Equivalence}\label{sec:pr-eq}

    Though precise, the prior definitions of equality are far too rigid in practice. The spaces involved either lack formal semantics or are intractable to exhaustive search. We now turn to a language for reasoning about probabilistic equivalence, which allows us to infer if and when functions are approximately equivalent.
