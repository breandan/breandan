...
%        \UnaryInfC{\textit{Commutivity}}
    ...
    \noindent For example, suppose we are given $f := a b c, f' := c b a$ and the rewrite system $\Sigma := \{(a, a), (ab, ba)\}$. Indeed, $\textsc{Eq}[f, f']$ can be established as follows:
...
%    a -> a [label="Σ₀"]
    ...
    $$\mathcal{O}_{\textsc{Eq}}(\underset{i \leq n}{\text{max }}\underset{n}{\text{min}}\{|E| \mid \Gamma^i_g \vdash E, \Gamma_g^n = \Gamma_g^{n+1}\})$$ % \text{, or } \Theta_{\textsc{Eq}}(|\mathcal{I}|!) \text{ for } \Sigma.$$
    ...
    Clearly, the procedure defined in \S~\ref{subsec:intensional-equivalence} is severely limited by $|\mathcal{I}|$ and $|\Sigma^*|$. While equality may be tractable to establish, disequality remains a challenge. For extensional equivalence, we will see the opposite holds, ceteris paribus.
