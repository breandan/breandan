...
%    a -> a [label="Σ₀"]
    ...
    \begin{prooftree}
        \AxiomC{$\Gamma \vdash f, g: \mathcal{I} \rightarrow \mathcal{I}$}
        \AxiomC{$\Sigma: \{\mathcal{I}\rightarrow \mathcal{I}\}\rightarrow \mathcal{I}$}
        \RightLabel{\textsc{(Init)}}
...
%        \AxiomC{$i: \mathcal{I}$}
        ...
        \DisplayProof
        \vskip 1em
        \AxiomC{$\Gamma \vdash \forall \Sigma \in \Sigma^* \mid f(\Sigma) \Downarrow f'(\Sigma) $}
        \RightLabel{\textsc{(Eq)}}
        \UnaryInfC{$\Gamma \vdash f = f'$}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma \vdash \exists \Sigma \in \Sigma^* \mid  f(\Sigma) \not\Downarrow f'(\Sigma) $}
        \RightLabel{\textsc{(Neq)}}
        \UnaryInfC{$\Gamma \vdash f \neq f'$}
    \end{prooftree}

    Suppose $\mathcal{I}$ has a normal form such that $\mathcal{O}(r_1\overset{?}{=}r_2) = \mathcal{O}(1), \forall i_1, i_2 \in \mathcal{I}$.




    ``Do $f_1$ and $f_2$ behave in the same way over all inputs?" While extensional equality is difficult to show if $|\mathcal{I}|$ is large, detecting inequality for pairs of random functions is relatively straightforward: we can simply search for $\hat i \in \mathcal{I}$ such that $f_1(\hat i) \neq f_2(\hat i)$.
