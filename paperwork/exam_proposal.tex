...
%    We can define semantic equality in our setting using word embeddings.
    ...
    Approximate equivalence requires a distance metric. To define a proper distance metric, we need the concept of a \textit{kernel function}. Suppose we have a feature map $\varphi: \mathcal{X} \rightarrow \mathcal{Y}$. A kernel function $k: \mathcal{X} \times \mathcal{X} \rightarrow \mathbb{R}$ defined as $k(f, g) \mapsto \left< \varphi(f), \varphi(g) \right>_\mathcal{Y}$ is a metric on $\mathcal{Y}$. Crucially, $\left< \cdot, \cdot \right>_\mathcal{Y}$ must be an inner product, i.e. Cauchy-Schwartz must hold.
