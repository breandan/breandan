...
%    To be more inclusive of our audience, we should optimize our definitions for consistency, at the expense of completeness.
    ...
    \pagebreak\section{Graph Representation}\label{sec:graphs}
   ...
   % https://en.wikipedia.org/wiki/Duality_(optimization)
    ...
    \pagebreak\section{Search-Based Software Engineering}\label{sec:applications}

    Knowledge systems are a collection of facts which have been verified by human beings. For example, we can treat mathematics as a knowledge base of rewrite rules~\citep{rich2009knowledge}. This has been successfully operationalized in the Theano~\citep{bergstra2010theano} system. Mathematicians encode their knowledge as a set of facts, and the compiler uses those facts to optimize computation graphs, e.g. for numerical stability.

    More generally, we can think of code as a procedural knowledge system, describing data transformations. We would like a way to reason about those transformations, for example to detect similar procedures. However the same algorithm implemented by two developers is seldom written the same way (citation needed). So we need some mechanism to detect exact or approximate equality in source code.
