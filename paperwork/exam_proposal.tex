...
%    We require a metric~\ref{sec:probabilistic}.
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{c|l|l|}
            ...
            \multicolumn{1}{|c|}{\textbf{Exact}}       & \begin{tabular}[c]{@{}l@{}}Type Checking\\ Model Checking\end{tabular}   & \begin{tabular}[c]{@{}l@{}}Variable Elimination\\Probabilistic Circuits \end{tabular} \\ \hline
            \multicolumn{1}{|c|}{\textbf{Approximate}} & \begin{tabular}[c]{@{}l@{}}Software Testing\\Dynamic Analysis\end{tabular} & \begin{tabular}[c]{@{}l@{}}Monte Carlo Methods\\Bayesian Networks\end{tabular}                         \\ \hline
        \end{tabular}
    \end{table}
...
%    Still, we can answer many questions exactly. If we restrict the reasoning system to incomplete queries, we can be consistent.
    ...
    Let $\Gamma_{f}: (\mathcal{I} \rightarrow \mathcal{O}) \times (\mathcal{I}\rightarrow \mathcal{O})$ be a relation on a family of functions which are closed under composition. We say that two functions $f, f': \mathcal{I} \rightarrow \mathcal{O}$ are intensionally equal under $\Gamma_f$ if $f' \in \Gamma^n_{f}$ for some $n \in \mathbb{Z}$.

    \begin{prooftree}
        \AxiomC{$f: \mathcal{I} \rightarrow \mathcal{O}$}
        \RightLabel{Init}
        \UnaryInfC{$\Gamma^0_{f} \vdash \{f\}$}
%        \UnaryInfC{\textit{Commutivity}}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma^{n}_{f} \vdash E$}
        \RightLabel{Sub}
        \UnaryInfC{$\Gamma^{n+1}_{f} \vdash \bigcup\limits_{\substack{e \in E\\\sigma \in \Sigma}}e[\sigma_1\rightarrow\sigma_2]$}
        \DisplayProof
        \vskip 1em
        \AxiomC{$\Gamma^n_{f} \vdash E$}
        \AxiomC{$f' \in E$}
        \RightLabel{Eq}
        \BinaryInfC{$\Gamma^n_{f} \vdash f = f'$}
        \DisplayProof
        \hskip 1em
        \AxiomC{$\Gamma^n_{f}\vdash E$}
    ...
    For example, suppose we are given $f \vdash x  y  z, f' \vdash z  y  x$ and the rewrite system $\Sigma := \{(a, a), (ab, ba)\}$. We can show $f = f'$ as follows:

    \begin{prooftree}
        \def\fCenter{\ \vdash\ }
        \Axiom$f \fCenter x  y  z$
        \RightLabel{$R_1$}
        \UnaryInf$\Gamma^0_{f} \fCenter \{xyz\}$
        \RightLabel{$R_2$}
        \UnaryInf$\Gamma^1_{f} \fCenter \{xyz, yxz, xzy\}$
        \RightLabel{$R_2$}
        \UnaryInf$\Gamma^2_{f} \fCenter \{xyz, yxz, xzy, yzx, zxy\}$
        \RightLabel{$R_2$}
        \UnaryInf$\Gamma^3_{f} \fCenter \{xyz, yxz, xzy, yzx, zxy, \mathbf{zyx}\}$
        \RightLabel{$R_3$}
        \UnaryInf$\Gamma^3_{f} \fCenter E \hskip 3em f' \vdash zyx \in E$
        \RightLabel{$R_3$}
        \UnaryInf$\Gamma^3_{f} \fCenter f=f'$
    \end{prooftree}

    Let us consider the complexity of checking $g\overset{?}{=}g'$ for arbitrary $g, g' \in \mathbb{Z}^k$ as $k$ grows. It is possible to show that $\Sigma$ terminates in

    $$\mathcal{O}(g, g', k) = \underset{g' \in \mathbb{Z}^k}{\text{min }}\{n \mid \Gamma_g^n(g, g') = \Gamma_g^{n+1}(g, g')\}$$

    Now let us consider the modification $\Sigma' := \Sigma \cup \{(a, 1a)\}$. It is possible to show that $\Sigma'$ no longer terminates for arbitrary $g, g' \in \mathbb{Z}^k$.
