...
%    \end{prooftree}
    ...
    So far, we have seen various notions of exact and approximate equivalence. However, what is missing is a way to quantify the probability a certain value or set of values is produced, or the likelihood of observing a representable function in the wild. To do this properly we need a language for constructing programs probabilistically. Probabilistic inference is one such framework for reasoning about uncertainty. In this section, we define a denotational and operational semantics for probabilistic inference.
...
\noindent Given a joint $P(X, Y)$, if we see an event $y: Y$, this observation is called \textit{conditioning} and the resulting distribution over $X$, a \textit{conditional distribution}:
...
\noindent Given a conditional distribution and its prior, to exchange the order of conditioning, we can use Bayes rule~\cite{bayes1763essay} :
...
\noindent When a conditional distribution $P(X \mid Y)$ does not depend on its prior, the variables are said to be \textit{independent}.
...
\noindent Equivalently, if two distributions $P(X)$ and $P(Y)$ are multiplied to form a joint distribution $P(X, Y)$, we may conclude that $X$ and $Y$ are independent:
...
Given a joint distribution $P(X, Y)$, in order to remove a variable, we must sum or integrate over its conditional distribution. This process is known as \textit{marginalization}, and the resulting distribution is called a \textit{marginal}:
...
%    How do we know when we are approaching equality? We can use a distance metric.
    ...
    Hypothesis testing, Kolmogorov-Smirnov, Kantorovich-Rubinstein, Kullback-Leibler, L\'evy-Prokhorov, Jensen-Shannon, EMD.
