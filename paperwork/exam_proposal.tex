...
%    \end{enumerate}
    ...
    In order to reason about software, we must define what is meant for software artifacts to be \textit{same} or \textit{different}. This notion has been studied in many logical disciplines. Various algorithms have been proposed to determine whether two artifacts are equal with respect to some definition of equality and set of axioms. For example, one might run them, rewrite them, or compare them in various contexts to ascertain their equivalence.
    ...
    The problem is, mathematical equality is not a differentiable operator. If two things are not equal, gradient alone cannot tell us how to make them so. When implementing a differentiable program using the expression $\texttt{if(i = j)}$, we are forced to use a definition which is not entirely sound. Consider the Kronecker delta function $\delta_k: \mathbb{T}$, where $\mathbb{T} \in \{\mathbb{Z, Q, B}\}$:
    ...
    Neither of these definitions is particularly appealing. Do we really need $\infty$ just to describe equality? Without knowing $i$ or $j$, how do we optimize such a function using gradient based techniques?
